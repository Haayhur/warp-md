...
impl Plan for RandomizeIonsPlan {
    ...
    fn process_chunk(
        ...
        if let (Device::Cuda(ctx), Some(gpu)) = (_device, &self.gpu) {
            if self.by > 0.0 && !self.solute_indices.is_empty() && !self.solvent_residues.is_empty()
            {
        ...
        for frame in 0..chunk.n_frames {
            ...
            for ix in 0..reps[0] {
                for iy in 0..reps[1] {
                    for iz in 0..reps[2] {
                        let sx =
                            ix as f64 * ax as f64 + iy as f64 * bx as f64 + iz as f64 * cx as f64;
                        let sy =
                            ix as f64 * ay as f64 + iy as f64 * by as f64 + iz as f64 * cy as f64;
                        let sz =
                            ix as f64 * az as f64 + iy as f64 * bz as f64 + iz as f64 * cz as f64;
                        for &idx in sel.iter() {
