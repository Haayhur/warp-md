...
use crate::plans::analysis::group_runtime::{
    fill_frame_group_positions, seed_unwrapped_groups, unwrap_frame_groups,
};
use crate::plans::analysis::time_correlation::{lag_allowed, update_time_axis};
...
use super::super::fft::conductivity_fft;
use super::super::runtime_math::{average_volume, build_conductivity_output};
use super::super::ConductivityPlan;

impl ConductivityPlan {
    pub(in super::super) fn process_chunk_state(
        &mut self,
        chunk: &FrameChunk,
        _system: &System,
        _device: &Device,
        ...
        for frame in 0..n_frames {
            ...
            if let Some([lx, ly, lz]) = orthorhombic_lengths(&chunk.box_[frame]) {
                let vol = lx * ly * lz * self.length_scale.powi(3);
                self.vol_sum += vol;
                self.vol_count += 1;
            }
    ...
    pub(in super::super) fn finalize_state(&mut self) -> TrajResult<PlanOutput> {
        if self.profile_transport {
            eprintln!(
