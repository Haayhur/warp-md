...
use super::sorient::{
    box_lengths, dot, minimum_image_between_atoms, normalize, reference_positions,
};
...
fn minimum_image_point_to_reference(
    ...
    let mut dx = atom[0] - reference[0];
    let mut dy = atom[1] - reference[1];
    let mut dz = atom[2] - reference[2];
    match box_ {
