...
use super::sorient::{dot, minimum_image_between_atoms, normalize};

pub struct HydOrderPlan {
    selection: Selection,
    axis: usize,
    bin: f64,
    tblock: usize,
...
impl HydOrderPlan {
    ...
    fn initialize_from_frame(&mut self, chunk: &FrameChunk, frame: usize) -> TrajResult<()> {
        ...
        if let Some(lengths) = box_lengths(chunk.box_[frame]) {
            self.used_box = true;
            self.origins = [0.0, 0.0, 0.0];
            for axis in 0..3 {
                let extent = lengths[axis] * self.length_scale;
                self.dims[axis] = resolve_bins(self.bin, extent)?;
...
fn resolve_bins(bin: f64, extent: f64) -> TrajResult<usize> {
    if !extent.is_finite() || extent <= 0.0 {
        return Err(TrajError::Mismatch(
            "hydorder requires positive spatial extent".into(),
        ));
    }
    Ok(((extent / bin) + 0.5).floor().max(1.0) as usize)
}
