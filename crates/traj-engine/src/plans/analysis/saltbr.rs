    ...
    fn process_chunk(
        ...
        for frame in 0..chunk.n_frames {
            ...
            for (pair_idx, [left, right]) in self.pair_indices.iter().copied().enumerate() {
                let (dx, dy, dz) =
                    distance_vec(centers[left as usize], centers[right as usize], box_l);
...
fn distance_vec(a: [f64; 3], b: [f64; 3], box_l: Option<[f64; 3]>) -> (f64, f64, f64) {
    ...
    let mut dy = b[1] - a[1];
    let mut dz = b[2] - a[2];
    if let Some(box_l) = box_l {
        if box_l[0] > 0.0 {
            dx -= (dx / box_l[0]).round() * box_l[0];
        }
        if box_l[1] > 0.0 {
            dy -= (dy / box_l[1]).round() * box_l[1];
        }
        if box_l[2] > 0.0 {
            dz -= (dz / box_l[2]).round() * box_l[2];
        }
    }
    (dx, dy, dz)
}
