...
use super::{
    mapping_mode, CgBead, CgOutputRequest, CgRequest, SourceMappingResult, AGENT_SCHEMA_VERSION,
};

#[derive(Clone, Debug)]
pub(super) struct TopologyRenderPolicy {
    pub(super) exclusions_mode: String,
    pub(super) exclusion_hops: usize,
    pub(super) dihedrals_enabled: bool,
}

impl TopologyRenderPolicy {
    pub(super) fn from_output(output: &CgOutputRequest) -> Self {
        Self {
            exclusions_mode: output
                .exclusions
                .as_ref()
                .map(|policy| policy.mode.clone())
                .unwrap_or_else(|| "explicit_all_intra".to_string()),
            exclusion_hops: output
                .exclusions
                .as_ref()
                .and_then(|policy| policy.n_hops)
                .unwrap_or(1)
                .max(1),
            dihedrals_enabled: output
                .dihedrals
                .as_ref()
    ...
    fn nrexcl(&self) -> usize {
        if self.exclusions_mode == "none" {
            0
        } else {
            1
        }
...
impl Default for TopologyRenderPolicy {
    fn default() -> Self {
        Self {
            exclusions_mode: "explicit_all_intra".to_string(),
            exclusion_hops: 1,
            dihedrals_enabled: false,
        }
    }
}
...
pub(super) fn render_martini_itp(
    ...
    render_martini_itp_with_policy(
        name,
        mapping,
        bond_stats,
        angle_stats,
        ...
        reference_targets,
        tuning,
        &TopologyRenderPolicy::default(),
    )
}

pub(super) fn render_martini_itp_with_policy(
    name: &str,
    mapping: &MappingResult,
    bond_stats: &[BondStats],
    angle_stats: &[AngleStats],
    ...
    out.push_str(&format!("{molecule:<16} {}\n\n", policy.nrexcl()));
    out.push_str("[ atoms ]\n");
    ...
    if let Some(targets) = reference_targets {
        render_grouped_bonded_sections(&mut out, targets, tuning, policy);
    ...
    if policy.dihedrals_enabled && reference_targets.is_none() && !dihedral_stats.is_empty() {
        out.push_str("\n[ dihedrals ]\n");
...
pub(super) fn render_source_martini_itp_with_policy(
    name: &str,
    source_mapping: &SourceMappingResult,
    bond_stats: &[BondStats],
    angle_stats: &[AngleStats],
    ...
    if policy.dihedrals_enabled && effective_targets.is_none() && !dihedral_stats.is_empty() {
        out.push_str("\n[ dihedrals ]\n");
        out.push_str("; i  j  k  l  funct  angle(deg)  force  multiplicity\n");
        for stat in dihedral_stats {
            let (phase, force) = bonded_dihedral_parameters(stat, tuning);
    ...
    render_exclusions(
        &mut out,
        source_mapping.mapping.bead_names.len(),
...
fn render_grouped_bonded_sections(
    ...
    if policy.dihedrals_enabled && !targets.dihedrals.is_empty() {
        out.push_str("\n[ dihedrals ]\n");
        out.push_str("; i  j  k  l  funct  angle(deg)  force  multiplicity\n");
...
fn render_exclusions(
    ...
    let pairs = exclusion_pairs(bead_count, connections, policy);
    if pairs.is_empty() {
        return;
    }
    out.push_str("\n[ exclusions ]\n");
...
fn exclusion_pairs(
    ...
    match policy.exclusions_mode.as_str() {
        "none" | "nrexcl" => Vec::new(),
        "explicit_all_intra" => {
            let mut pairs = Vec::new();
            for left in 0..bead_count {
                for right in left + 1..bead_count {
                    pairs.push((left, right));
                }
            }
            pairs
        }
        "explicit_nhop" => explicit_nhop_pairs(bead_count, connections, policy.exclusion_hops),
        _ => Vec::new(),
    }
}

fn explicit_nhop_pairs(
    bead_count: usize,
    connections: &[(usize, usize)],
    n_hops: usize,
) -> Vec<(usize, usize)> {
    let mut adjacency = vec![Vec::<usize>::new(); bead_count];
    ...
    for start in 0..bead_count {
        let mut seen = vec![false; bead_count];
        seen[start] = true;
