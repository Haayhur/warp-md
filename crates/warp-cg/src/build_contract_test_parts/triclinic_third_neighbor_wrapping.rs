...
#[test]
fn triclinic_region_wrapping_finds_third_neighbor_patch_image() {
    ...
    let region = LeafletRegion {
        name: Some("third-image".to_string()),
        role: "patch".to_string(),
        geometry: RegionGeometry::Polygon {
            points_angstrom: vec![
                basis.cartesian([3.00, 0.46]),
                basis.cartesian([3.10, 0.46]),
                basis.cartesian([3.10, 0.54]),
                basis.cartesian([3.00, 0.54]),
            ],
            scale_xy: None,
            rotate_degrees: 0.0,
        },
