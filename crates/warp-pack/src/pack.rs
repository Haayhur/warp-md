...
use crate::spatial_hash::{SpatialHash, SpatialHashParamsExt};
...
fn rebuild_hash_from_positions(hash: &mut SpatialHash, positions: &[Vec3], cell_size: f32) {
        ...
        for (spec_index, spec) in cfg.structures.iter().enumerate() {
            ...
            for _ in 0..loops0 {
                ...
                if !cfg.disable_movebad && base_stats.max_constraint > precision {
                    rebuild_hash_from_positions(&mut hash, &positions, cell_size);
