...
pub fn run_from_env() -> Result<u8, String> {
    run_from_args(std::env::args_os())
}

pub fn run_from_args<I, T>(args: I) -> Result<u8, String>
where
    ...
    T: Into<OsString> + Clone,
{
    match Cli::try_parse_from(args) {
        ...
        Err(err) => {
            let kind = err.kind();
            err.print().map_err(|print_err| print_err.to_string())?;
            match kind {
                ErrorKind::DisplayHelp | ErrorKind::DisplayVersion => Ok(0),
                _ => Ok(2),
            }
        }
    }
    ...
    match cli.cmd {
        Command::Schema {
            ...
            let value: serde_json::Value =
                serde_json::from_str(&crate::schema_json(&kind)?).map_err(|err| err.to_string())?;
        ...
        Command::Example {
            ...
            print_value(
                &crate::example_request(&engine, &task),
        ...
        Command::Validate {
            ...
            let (code, value) = crate::validate_request_json(&text);
        ...
        Command::Run { request, stream } => {
            ...
            let (code, value) = crate::run_request_json(&text, stream);
            if !stream {
        ...
        Command::InspectOutput {
            ...
            let (code, value) = crate::inspect_output_json(&output.to_string_lossy(), &engine);
            print_value(&value, selected_format(format, json))?;
            Ok(code as u8)
        }
