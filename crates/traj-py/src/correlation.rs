...
#[pyclass]
struct PyAtomicCorrelationPlan {
    plan: RefCell<AtomicCorrelationPlan>,
}

#[pymethods]
impl PyAtomicCorrelationPlan {
    #[new]
    ...
    fn new(
        ...
        let reference_mode = parse_reference(reference)?;
        let mut plan = AtomicCorrelationPlan::new(selection.selection.clone(), reference_mode);
        if let Some(mode) = lag_mode {
            plan = plan.with_lag_mode(parse_lag_mode(mode)?);
        }
        if let Some(max_lag) = max_lag {
...
#[pyclass]
struct PyCrossCorrelationPlan {
    plan: RefCell<CrossCorrelationPlan>,
}

#[pymethods]
impl PyCrossCorrelationPlan {
    #[new]
    ...
    fn new(
        ...
        let reference_mode = parse_reference(reference)?;
        let mut plan = CrossCorrelationPlan::new(
...
impl PyCurrentPlan {
    ...
    fn new(
        ...
        let group_by = parse_group_by(group_by)?;
        let mut plan =
            CurrentPlan::new(selection.selection.clone(), group_by, charges, temperature)
...
#[pyclass]
struct PyWaterOrderPlan {
    plan: RefCell<WaterOrderPlan>,
}

#[pyclass]
struct PyHydrationOrderPlan {
    plan: RefCell<HydrationOrderPlan>,
}

#[pyclass]
struct PyHelixOrientationPlan {
    plan: RefCell<HelixOrientationPlan>,
}

#[pyclass]
struct PySolventOrientationPlan {
    plan: RefCell<SolventOrientationPlan>,
}

#[pyclass]
struct PySolventPolarizationPlan {
...
impl PyWaterOrderPlan {
    ...
    fn new(
        ...
        let mut plan = WaterOrderPlan::new(
            oxygen_indices_u32,
    ...
    fn run<'py>(
        ...
        match output {
            PlanOutput::H2Order(output) => water_order_to_py(py, output),
            _ => Err(PyRuntimeError::new_err("unexpected output")),
        }
    }
}
...
impl PyHydrationOrderPlan {
    ...
    fn new(
        ...
        let axis = parse_axis(axis)?;
        let mut plan = HydrationOrderPlan::new(selection.selection.clone(), axis, bin)
            .with_tblock(tblock)
    ...
    fn run<'py>(
        ...
        match output {
            PlanOutput::HydOrder(output) => hydration_order_to_py(py, output),
            _ => Err(PyRuntimeError::new_err("unexpected output")),
        }
    }
}
...
impl PyHelixOrientationPlan {
    ...
    fn new(
        ...
        let mut plan =
            HelixOrientationPlan::new(ca_selection.selection.clone()).with_incremental(incremental);
    ...
    fn run<'py>(
        ...
        match output {
            PlanOutput::HelixOrient(output) => helix_orientation_to_py(py, output),
            _ => Err(PyRuntimeError::new_err("unexpected output")),
        }
    }
}
...
impl PySolventOrientationPlan {
    ...
    fn new(
        ...
        let mut plan = SolventOrientationPlan::new(
    ...
    fn run<'py>(
        ...
        match output {
            PlanOutput::SOrient(output) => solvent_orientation_to_py(py, output),
            _ => Err(PyRuntimeError::new_err("unexpected output")),
        }
    }
}
...
#[pymethods]
impl PySolventPolarizationPlan {
    #[new]
    ...
    fn new(
        ...
        let mut plan = SolventPolarizationPlan::new(
            solute_selection.selection.clone(),
            atom1_indices_u32,
            atom2_indices_u32,
            atom3_indices_u32,
    ...
    fn run<'py>(
        ...
        match output {
            PlanOutput::Spol(output) => solvent_polarization_to_py(py, output),
            _ => Err(PyRuntimeError::new_err("unexpected output")),
        }
    }
}
...
pub(crate) fn register(m: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    m.add_class::<PyAtomicCorrelationPlan>()?;
    m.add_class::<PyVelocityAutoCorrPlan>()?;
    m.add_class::<PyVanHovePlan>()?;
    m.add_class::<PyCrossCorrelationPlan>()?;
    ...
    m.add_class::<PyWaterOrderPlan>()?;
    m.add_class::<PyHydrationOrderPlan>()?;
